...
func main() {
	...
	fmt.Println("=== Jupiter Ultra 交易执行器 ===")
	...
	if *inputMint != "" {
		...
		fmt.Printf("使用命令行参数 inputMint: %s\n", finalInputMint)
	} else if defaultInputMint != "" {
		finalInputMint = defaultInputMint
		fmt.Printf("使用环境变量 inputMint: %s\n", finalInputMint)
	} else {
	...
	if finalInputMint == "So11111111111111111111111111111111111111112" {
		...
		if defaultOutputMint != "" {
			...
			fmt.Printf("使用环境变量 outputMint: %s\n", finalOutputMint)
		} else {
		...
		fmt.Printf("自动卖出模式已启用，检测到代币输入，正在查找余额...\n")
			...
			if tokenMint == finalInputMint {
				for _, account := range tokenAccounts {
					if account.UIAmount > 0 {
						...
						found = true
						fmt.Printf("找到代币余额: %s (%.6f)\n", account.Amount, account.UIAmount)
						break
					}
				}
				break
	...
	if *maxFee != "" {
		...
		fmt.Printf("使用命令行参数 MAX_PRIORITIZATION_FEE_LAMPORTS: %s\n", *maxFee)
	}
...
func showHelp() {
	fmt.Println("=== Jupiter Ultra 交易执行器 ===")
	fmt.Println("用法:")
	fmt.Println("  go run .                           - 运行交易执行器")
	fmt.Println("  go run . encrypt                   - 加密PRIVATE_KEY")
	fmt.Println("  go run . help                      - 显示帮助信息")
	fmt.Println("")
	fmt.Println("命令行参数:")
	fmt.Println("  -input string                      - 输入代币的mint地址 (优先级最高)")
	fmt.Println("  -maxfee string                     - 最大优先费用 (lamports)")
	fmt.Println("  -help                              - 显示帮助信息")
	fmt.Println("")
	fmt.Println("参数优先级:")
	fmt.Println("  1. 命令行参数 (最高优先级)")
	fmt.Println("  2. 环境变量 (swap.env文件)")
	fmt.Println("  3. 用户输入")
	fmt.Println("")
	fmt.Println("示例:")
	fmt.Println("  go run . -input So11111111111111111111111111111111111111112")
	fmt.Println("  go run . -input your_token_mint_address -maxfee 1000000")
}
